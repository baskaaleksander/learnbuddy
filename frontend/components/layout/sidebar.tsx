...
function Sidebar({ paths, isOpen, toggle, isMobile = false }: SidebarProps) {
  ...
  if (!isMobile) {
    return (
      <div
        className={cn(
          "sticky top-0 min-h-screen h-screen border-r border-gray-200 dark:border-gray-800 bg-background transition-all duration-300 ease-in-out hidden md:flex md:flex-col",
          isOpen ? "w-64" : "w-16"
        )}
      >
        <div className="">
              ...
              {category.links.map((link, linkIndex) => {
                ...
                const linkElement = (
                  ...
                  >
                    <Tooltip>
                      <TooltipTrigger>
                        <div className={cn("text-lg", !isOpen && "mx-auto")}>
                          {link.icon}
                        </div>
                      </TooltipTrigger>
                      <TooltipContent
