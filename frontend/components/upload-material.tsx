...
function UploadMaterial() {
  ...
  const createPdfFromText = async (text: string, title: string): Promise<Blob> => {
    const docDefinition = {
      info: {
        title: title,
  ...
  const onSubmit = async (data: z.infer<typeof formSchema>) => {
    try {
      ...
      if (data.textContent && !data.pdfFile) {
        const blob = await createPdfFromText(data.textContent, `${user?.email}-material-${Date.now().toLocaleString}.pdf`);
        fileToUpload = new File([blob], `${user?.email}-material-${Date.now()}.pdf`, { type: 'application/pdf' });
                  ...
                  >
                    ...
                    {fileName ? (
                      <div className="flex flex-col items-center justify-center">
                        ...
                        <p className="text-sm text-gray-500 mt-1">Click or drop another file to replace</p>
                      </div>
                    ) : (
                      <div className="flex flex-col items-center justify-center">
