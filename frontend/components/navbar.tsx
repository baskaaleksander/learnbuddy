...
function DesktopNavbar({ user, logout, navLinks, pathname } : { user: UserData | null , logout: () => void, navLinks: { name: string, path: string }[], pathname: string }) {
  return (
...
function MobileNavbar({ user, logout, navLinks, pathname } : { user: UserData | null , logout: () => void, navLinks: { name: string, path: string }[], pathname: string }) {
  ...
  return (
    <>
      <nav className='flex sm:hidden justify-between items-center border-b border-gray-200 dark:border-gray-800 w-full bg-background px-4 py-2 fixed top-0 left-0 right-0 z-50'>
        <Image src={logo} alt="LearnBuddy" width={140} height={40} className="w-[140px] lg:w-[180px]" />
            ...
            >
              <ul className='flex flex-col items-center space-y-8 w-full'>
                ...
                {user && (
                  <motion.li 
                    ...
                    initial={{ opacity: 0, y: 10 }}
                    animate={{ opacity: 1, y: 0 }}
                    transition={{ duration: 0.2, delay: 0.1 + navLinks.length * 0.1 }}
                  >
                    <Link 
                      href="/dashboard" 
                      className={`block font-semibold text-xl py-2 transition-colors ${
              ...
              >
                {user ? (
                  ...
                  <>
                    <Button variant="default" className='w-full text-base py-6' onClick={() => setIsOpen(false)}>
                      <Link href="/login" className="w-full">Login</Link>
                    </Button>
                    <Button variant="outline" className='w-full text-base py-6' onClick={() => setIsOpen(false)}>
